...
#define SOCKET_NAME_RIL_SIM_ONE "rild-one"
#define SOCKET_NAME_RIL_SIM_TWO "rild-two"
#define SOCKET_ONE 1
#define SOCKET_TWO 2
...
static int s_fdListenDsds = -1;
static int s_fdCommand = -1;
static int s_fdCommandDsds = -1;
...
static struct ril_event s_commands_event_dsds;
static struct ril_event s_wakeupfd_event;
static struct ril_event s_listen_event;
static struct ril_event s_listen_event_dsds;
...
static void setSimDsds(Parcel &p, RequestInfo *pRI);
...
static int responseIncomingMessage(Parcel &p, void *response, size_t responselen);
...
dispatchInts (Parcel &p, RequestInfo *pRI) {
   ...
   if (pRI->pCI->requestNumber == RIL_REQUEST_RADIO_POWER) {
	if (socket == SOCKET_TWO) {
	   s_callbacks.onRequest(RIL_REQUEST_RADIO_POWER_SIM_TWO, const_cast<int *>(pInts),
                       datalen, pRI);	
	}
   }
...
/**
 * Set SIM request from user
 * 
 * sim number passed in parcel	
 * 
 */
static void setSimDsds(Parcel &p, RequestInfo *pRI){
    ...
    clearPrintBuf;
    printRequest(pRI->token, pRI->pCI->requestNumber);
    s_callbacks.onRequest(pRI->pCI->requestNumber, &sim, 0, pRI);
...
static int
sendResponseRawDsds (const void *data, size_t dataSize) {
    int fd = s_fdCommandDsds;
    int ret;
    uint32_t header;

    if (s_fdCommandDsds < 0) {
        return -1;
    }
...
sendResponse (Parcel &p) {
    printResponse;
	if (socket == SOCKET_TWO) {
		return sendResponseRawDsds(p.data(), p.dataSize());
	}
	else{
...
/**
 * DSDS Changes
 * modified method of including SIM number
 *
 * NULL strings are accepted
 * FIXME currently ignores responselen
 */
static int responseIncomingMessage(Parcel &p, void *response, size_t responselen) {
    ...
    if(s_callbacks.getDsdsSms()){
	p.writeInt32(s_callbacks.getDsdsSmsSim());
	socket = s_callbacks.getDsdsSmsSim();
    }
...
static int responseCallList(Parcel &p, void *response, size_t responselen) {
    ...
    for (int i = 0 ; i < num ; i++) {
 	...
 	socket = p_cur->simDsds;
        p.writeInt32(p_cur->simDsds);
...
static void processCommandsCallbackDsds(int fd, short flags, void *param) {
    RecordStream *p_rs;
    void *p_record;
    size_t recordlen;
    int ret;

    assert(fd == s_fdCommandDsds);
    ...
    if (ret == 0 || !(errno == EAGAIN || errno == EINTR)) {
        /* fatal error or end-of-stream */
        if (ret != 0) {
            LOGE("error on reading command socket2 errno:%d\n", errno);
        } else {
            LOGW("EOS.  Closing command socket2.");
        }

        close(s_fdCommandDsds);
        s_fdCommandDsds = -1;

        ril_event_del(&s_commands_event_dsds);

        record_stream_free(p_rs);

        /* start listening for new connections again */
        rilEventAddWakeup(&s_listen_event_dsds);
...
static void onNewCommandConnectDsds() {
    // Inform we are connected and the ril version
    int rilVer = s_callbacks.version;
	socket = SOCKET_TWO;
    RIL_onUnsolicitedResponse(RIL_UNSOL_RIL_CONNECTED,
...
static void listenCallbackDsds (int fd, short flags, void *param) {
    int ret;
    int err;
    int is_phone_socket;
    RecordStream *p_rs;
    ...
    assert (s_fdCommandDsds < 0);
    assert (fd == s_fdListenDsds);

    s_fdCommandDsds = accept(s_fdListenDsds, (sockaddr *) &peeraddr, &socklen);
    ...
    err = getsockopt(s_fdCommandDsds, SOL_SOCKET, SO_PEERCRED, &creds, &szCreds);
    ...
    ret = fcntl(s_fdCommandDsds, F_SETFL, O_NONBLOCK);

    if (ret < 0) {
        LOGE ("Error setting O_NONBLOCK errno:%d", errno);
    }

    LOGI("libril: new connection two");

    p_rs = record_stream_new(s_fdCommandDsds, MAX_COMMAND_BYTES);

    ril_event_set (&s_commands_event_dsds, s_fdCommandDsds, 1,
        processCommandsCallbackDsds, p_rs);

    rilEventAddWakeup (&s_commands_event_dsds);

    onNewCommandConnectDsds();
}
...
RIL_register (const RIL_RadioFunctions *callbacks) {
...
#else
    s_fdListen = android_get_control_socket(SOCKET_NAME_RIL_SIM_ONE);
    if (s_fdListen < 0) {
        LOGE("Failed to get socket '" SOCKET_NAME_RIL_SIM_ONE "'");
        exit(-1);
    }
    ...
    s_fdListenDsds = android_get_control_socket(SOCKET_NAME_RIL_SIM_TWO);
    if (s_fdListenDsds < 0) {
        LOGE("Failed to get socket '" SOCKET_NAME_RIL_SIM_TWO "'");
        exit(-1);
    }

    retDsds = listen(s_fdListenDsds, 4);

    if (retDsds < 0) {
        LOGE("Failed to listen on control socket2 '%d': %s",
             s_fdListenDsds, strerror(errno));
        exit(-1);
    }
#endif
    ...
    ril_event_set (&s_listen_event_dsds, s_fdListenDsds, false,
                listenCallbackDsds, NULL);
...
*/
    switch(request) {
        ...
        case RIL_REQUEST_RADIO_POWER_SIM_TWO: return "RADIO_POWER_SIM_TWO";
