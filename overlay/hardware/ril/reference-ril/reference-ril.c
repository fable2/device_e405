...
static int getDsdsSmsSim();

static int getDsdsSms();
...
static RIL_RadioState sStateDsds = RADIO_STATE_OFF;
...
static int callFromCLCCLine(char *line, RIL_Call *p_call)
{
    ...
    if (at_tok_hasmore(&line)) {
        ...
        err = at_tok_nextint(&line, &p_call->simDsds);
        if (err < 0) goto error;
    }
...
static void requestRadioPowerDsds(void *data, size_t datalen, RIL_Token t)
{
    int onOff;
    ...
    if (onOff == 0 && sStateDsds != RADIO_STATE_OFF) {
        err = at_send_command("AT+CFUN=0", &p_response);
       if (err < 0 || p_response->success == 0) goto error;
        setRadioState(RADIO_STATE_OFF);
    } else if (onOff > 0 && sStateDsds == RADIO_STATE_OFF) {
        err = at_send_command("AT+CFUN=1", &p_response);
        if (err < 0|| p_response->success == 0) {
            // Some stacks return an error when there is no SIM,
            // but they really turn the RF portion on
...
static int getDsdsSms()
{
    int dsdsSms = DSDS_SMS;

    return dsdsSms;

}

static void setSimDsds(void *data, RIL_Token t)
{    
    int* dataDsds;
...
onRequest (int request, void *data, size_t datalen, RIL_Token t)
{
    ...
    switch (request) {
        ...
        case RIL_REQUEST_CHANGE_SIM_PIN2:
            ...
            break;
	case RIL_REQUEST_CHANGE_ACTIVE_SIM:
    		setSimDsds(data, t);
	    	break;
    case RIL_REQUEST_RADIO_POWER_SIM_TWO:
            requestRadioPowerDsds(data, datalen, t);
            break;
        default:
            RIL_onRequestComplete(t, RIL_E_REQUEST_NOT_SUPPORTED, NULL, 0);
            break;
...
static void onUnsolicited (const char *s, const char *sms_pdu)
{
	...
	if(strStartsWith(s, "RING_ONE"))
	DSDS_SMS_SIM = 1;
	if(strStartsWith(s, "RING_TWO"))
	DSDS_SMS_SIM = 2;
	...
	if(strStartsWith(s, "+CMT: 1"))
	DSDS_SMS_SIM = 1;
	if(strStartsWith(s, "+CMT: 2"))
	DSDS_SMS_SIM = 2;
